...
def create_lv(path, volume_type="segmentation", array_name="raw"):
    ...
    return neuroglancer.LocalVolume(arr,
                                    dimensions=dims,
...
def add_example_layers(state, snapshot_path, add_time=True):
    ...
    for layer_name, layer_type in layers.items():
        if layer_type == "image":
            state.layers[layer_name] = neuroglancer.ImageLayer(source=lv_func(snapshot_path, volume_type=layer_type, array_name=layer_name))
        else:
            state.layers[layer_name] = neuroglancer.SegmentationLayer(source=lv_func(snapshot_path, volume_type=layer_type, array_name=layer_name))
...
def create_lv_stacked(snapshot_path, volume_type="segmentation", array_name="raw"):
    ...
    return neuroglancer.LocalVolume(dask_arrs,
                                    dimensions=dims,
                                    volume_type=volume_type,
                                    voxel_offset=voxel_offset)
