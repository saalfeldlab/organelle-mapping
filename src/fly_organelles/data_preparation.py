...
def filter_crops_for_percent_annotated(
    datasets, sampling, labels, threshold_percent_annotated=25
):
    new_datasets = copy.deepcopy(datasets)
    for dataset, ds_info in datasets["datasets"].items():
        new_crops = []
...
def _find_duplicates(data_config):
    ...
    for key, ds_info in datas["datasets"].items():
        ...
        for crop in ds_info["labels"]["crops"]:
            for c in crop.split(","):
                crop_path = (
                    Path(ds_info['labels']['data']) / ds_info['labels']['group'] / c
                )
                crop_root = fst.read(crop_path)
                example_class = crop_root.attrs["cellmap"]["annotation"]["class_names"][
                    0
                ]
                crop_datasets = crop_root[example_class].attrs["multiscales"][0][
                    "datasets"
                ]
        ...
        if duplicates:
            ...
            for crops in duplicates.values():
                ...
                if len(set(duplicate_scales)) > 1:
                    logger.info(
                        f"Different scales for duplicate crops {crops}: {duplicate_scales}"
                    )
                duplicate_sizes = [sizes[crop] for crop in crops]
                if len(set(duplicate_sizes)) > 1:
                    logger.info(
                        f"Different sizes for duplicate crops {crops}: {duplicate_sizes}"
                    )
        else:
...
class Crop:
    ...
    def get_coords(self, scale_level="s0"):
        some_label = next(iter(self.get_annotated_classes()))
        some_xarr = fst.read_xarray(
            os.path.join(self.crop_path, some_label, scale_level)
        )
    ...
    def get_annotation_type(self, label: str, scale_level="s0") -> str:
        return self.crop_root[label][scale_level].attrs["cellmap"]["annotation"][
            "annotation_type"
        ]["type"]

    def get_attributes(
        self, label: Optional[str] = None, scale_level: Optional[str] = "s0"
    ) -> dict():
    ...
    def get_encoding(self, label: str) -> dict[str, int]:
        return self.crop_root[label].attrs["cellmap"]["annotation"]["annotation_type"][
            "encoding"
        ]
    ...
    def create_new_class(self, atoms: set[str]):
        ...
        for l in self.get_annotated_classes():
            ...
            if l_set == atoms:
                ...
                raise ValueError(msg)
            if l_set < atoms:
                n_arr[self.get_array(l) == label_encoding["present"]] = encoding[
                    "present"
                ]
                if len(l_set) > 1:
                    subcombos.append(l)
        ...
        for combo in utils.all_combinations(subcombos):
            ...
            if missing <= self.get_annotated_classes():
                n_arr[
                    np.logical_and.reduce(
                        [
                            self.get_array(c) == self.get_encoding(c)["absent"]
                            for c in missing.union(combo)
                        ]
                    )
                ] = encoding["absent"]
    ...
    def save_class(
        ...
        multi = {
            m.name: m
            for m in multiscale(
                xarr, windowed_mode, (2, 2, 2), chunks=self.get_chunking()
            )
        }
        label_array_specs = {}
        ...
        annotation_group_attrs = AnnotationGroupAttrs(
            class_name=name, description="", annotation_type=annotation_type
        )
        ...
        if name not in self.get_annotated_classes():
            ...
            atts["cellmap"]["annotation"]["class_names"] = atts["cellmap"][
                "annotation"
            ]["class_names"] + [name]
    ...
    def convert_to_semantic(self, label: Optional[str] = None):
        if label is None:
            ...
            present_mask = np.logical_not(
                np.logical_or(
                    arr == label_encoding["absent"], arr == label_encoding["unknown"]
                )
            )
            encoding = {"absent": 0, "unknown": 255, "present": 1}
    ...
    def smooth_multiscale(self, label: Optional[str] = None):
        if label is None:
            ...
            try:
                annotation_types = [
                    self.get_annotation_type(label, sc) for sc in scalelevels
                ]
                done = [
                    annotype == "smooth_semantic_segmentation"
            ...
            full_scale_histo = self.get_attributes(label, full_scale)["cellmap"][
                "annotation"
            ]["complement_counts"]
            full_scale_nvals = np.prod(self.get_shape(full_scale))
            full_scale_histo["present"] = (
                full_scale_nvals
                - full_scale_histo.get("absent", 0)
                - full_scale_histo.get("unknown", 0)
            )
            ...
            for l1, l2 in itertools.pairwise(self.get_scalelevels(label)):
                ...
                if multi_valued:
                    ...
                    down = skimage.transform.downscale_local_mean(src, 2).astype(
                        "float32"
                    )
                    downslice = tuple(
                        slice(sh) for sh in (np.array(down.shape) // 2) * 2
                    )
                    down = down[downslice]
                    down[down > max(encoding["present"], encoding["absent"])] = (
                        encoding["unknown"]
                    )
                    histo = {}
                    histo["absent"] = round(
                    ...
                    for key in ["absent", "unknown"]:
                        histo[key] = (
                            counts[list(ids).index(encoding[key])]
                            if encoding[key] in ids
                            else 0
                        )
                    annotation_type = SemanticSegmentation(encoding=encoding)
                    annotation_array_attrs = AnnotationArrayAttrs(
                ...
                self.crop_root[label][l2].attrs.update(
                    wrap_attributes(annotation_array_attrs).model_dump()
                )
...
def _add_class_to_all_crops_func(
    label_config: BinaryIO, data_config: BinaryIO, new_label: str
):
    datas = yaml.safe_load(data_config)
    classes = utils.read_label_yaml(label_config)
...
def _convert_class_to_semantic_func(
    label_config: BinaryIO, data_config: BinaryIO, label: Optional[str] = None
):
    datas = yaml.safe_load(data_config)
    classes = utils.read_label_yaml(label_config)
...
def edit_offset_func(crop_path, orig_offset, new_offset, *, rounding: int = -1):
    ...
    for lbl in crop_root.keys():
        ...
        for ds in range(len(ms_attrs[0]["datasets"])):
            for k in range(
                len(ms_attrs[0]["datasets"][ds]["coordinateTransformations"])
            ):
                if (
                    ms_attrs[0]["datasets"][ds]["coordinateTransformations"][k]["type"]
                    == "translation"
                ):
                    transl = ms_attrs[0]["datasets"][ds]["coordinateTransformations"][
                        k
                    ]["translation"]
                    set_offset = np.array(transl) - diff_offset
                    if rounding >= 0:
                        set_offset = np.round(set_offset, rounding)
                    ms_attrs[0]["datasets"][ds]["coordinateTransformations"][k][
                        "translation"
                    ] = list(set_offset)
...
def fix_offset(data_config: BinaryIO, *, dry_run: bool = False):
    ...
    for key, ds_info in datas["datasets"].items():
        ...
        try:
            ...
            logger.info(
                "May have different metadata format. Proceeding with next dataset"
            )
            continue
        for ds in datasets:
            if ds["path"] == "s0":
        ...
        for crop in ds_info["labels"]["crops"]:
            for cropname in crop.split(","):
                logger.info(
                    f"Processing {ds_info['labels']['data']}/{ds_info['labels']['group']}/{cropname}"
                )
                crop_path = (
                    Path(ds_info['labels']['data'])
                    / ds_info['labels']['group']
                    ...
                    if cds["path"] == "s0":
                        ...
                        if utils.valid_offset(crop_off_arr, raw_res_arr, crop_res_arr):
                            ...
                            if np.all(crop_res < raw_res):
                                crop_off_arr_suggested = (
                                    crop_off_arr - crop_res_arr / 2.0
                                )
                            ...
                            if utils.valid_offset(
                                ...
                                logger.info(
                                    f"Crop {cropname} cannot be automatically corrected."
                                )
    click.echo(summary)
    return summary
